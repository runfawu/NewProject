...
- (void)connectionDidFinishLoading:(NSURLConnection *)aConnection
{
    ...
    {
        /*by linjunlong 2013-11-11  对SDImage图片进行压缩*/
        /*
        if(imageData.length>1024*200){
        NSData *takeimageData = UIImageJPEGRepresentation([UIImage imageWithData:imageData], 0.2);
        imageData=[(NSMutableData*)takeimageData retain];
        NSLog(@"0 imageData length is %d",[imageData length]);
        }
         */
        
        /*
        UIImage *tempImage=[UIImage imageWithData:imageData];
        if(tempImage.size.width>1024){
            tempImage=[tempImage clipImageWithScaleWithsize:CGSizeMake(640, 640*tempImage.size.height/tempImage.size.width)];
        }
        if(tempImage.size.height>1024){
            tempImage=[tempImage clipImageWithScaleWithsize:CGSizeMake(1024*tempImage.size.width/tempImage.size.height,1024)];
        }
        NSLog(@"0 imageData length is %d",[imageData length]);
        imageData=[(NSMutableData*)UIImageJPEGRepresentation(tempImage, 1.0) retain];
        */
