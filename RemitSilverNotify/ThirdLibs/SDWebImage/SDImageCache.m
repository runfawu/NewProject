...
- (void)queryDiskCacheForKey:(NSString *)key delegate:(id <SDImageCacheDelegate>)delegate userInfo:(NSDictionary *)info
{
    ...
    if (image)
    {
        /*by linjunlong 2013-11-11 SDImage 压缩图片*/
        /*
        NSData *theImageData = UIImageJPEGRepresentation(image, 1.0);
        if(theImageData.length>1024*150){
        NSData *takeimageData=UIImageJPEGRepresentation(image, 0.1);
        NSLog(@"takeimageData length is %d",[takeimageData length]/1024);
        image=[UIImage imageWithData:takeimageData];
        }
        */
        /*
        if(image.size.width>1024){
            image=[image clipImageWithScaleWithsize:CGSizeMake(640, 640*image.size.height/image.size.width)];
        }
        if(image.size.height>1024){
            image=[image clipImageWithScaleWithsize:CGSizeMake(1024*image.size.width/image.size.height,1024)];
        }
        NSData *takeimageData=UIImageJPEGRepresentation(image, 1.0);
        NSLog(@"image.size.width is %f,image.size.height %f",image.size.width,image.size.height);
        NSLog(@"takeimageData length is %d",[takeimageData length]/1024);
        */
